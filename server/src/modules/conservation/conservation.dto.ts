...
export class MessageDto {
  constructor(message: MessageDocument) {
    ...
    this.reactions = message.reactions.map(reaction => new ReactionDto(reaction));
  }
  _id: string;
  content: string;
...
export class ConservationDto {
  constructor(conservation: ConservationDocument) {
    ...
    this.members = conservation.members.map(user => new UserResponseDto(user));
    this.messages = conservation.messages.map(message => new MessageDto(message));
